...
**Prerequisites:** Docker running locally. Tests need PostgreSQL: `docker compose up -d db` (after Task 10) or any local PG with a `users_test` database reachable at `postgresql+asyncpg://postgres:postgres@localhost:5433/users_test`. During Tasks 1–9, start one with:
`docker run -d --name users-api-pg -e POSTGRES_PASSWORD=postgres -e POSTGRES_DB=users -p 5433:5432 postgres:16-alpine && docker exec users-api-pg psql -U postgres -c 'CREATE DATABASE users_test'`
...
```
DATABASE_URL=postgresql+asyncpg://postgres:postgres@localhost:5433/users
...
services:
  db:
    ...
    environment:
      ...
      POSTGRES_DB: users
    ports:
      - "5433:5432"  # host 5433: 5432 is taken by another local project
    volumes:
...
Run (stop the Task-1 throwaway postgres first if it holds port 5433: `docker rm -f users-api-pg`):
```bash
